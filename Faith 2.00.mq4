...
#property link      "blackalgotechnologies.com"
   ...
   /* 
      ...
      If Current Volatility (ATR(20)) is less than Volatility (ATR(20)) 10 hours ago:
         Market Long Order: closing price crosses lower Keltner Channel(20) from bottom
         Market Short Order: closing price crosses upper Keltner Channel(20) from top

      FAITH EXIT RULES:
      Time-based Exit: Close any order after 10 periods regardless whether in profit or loss
      TakeProfit Order: Place hard TakeProfit order 2 * ATR(20) away from opening price when trade is first executed
      StopLoss Order: Place hard StopLoss order 1 * ATR(20) away from opening price when trade is first executed
...
extern int     Entry_Keltner_Width     = 3;
...
extern double  TP_ATR_Multiplier = 2;
...
double keltnerUpper1, keltnerLower1, keltnerUpper2, keltnerLower2;
double close1, close2;
double ATR_Current, ATR_Past;
double ATR_SL, ATR_TP;
int LongEntryReversion, ShortEntryReversion;
int OrderNumber;
...
int start()
  {
      ...
      keltnerUpper1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, Entry_Keltner_Width, True, 0, 1); //Upper line, shift 1
      keltnerLower1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, Entry_Keltner_Width, True, 2, 1); //Lower line, shift 1
      ...
      LongEntryReversion  = Crossed1(close1, keltnerLower1);
      ShortEntryReversion = Crossed2(close1, keltnerUpper1);
   ...
   if(IsLossLimitBreached(IsLossLimitActivated, LossLimitPercent, OnJournaling, EntrySignal(ATR_Current, ATR_Past, LongEntryReversion, ShortEntryReversion)) == False)
   if(IsMaxPositionsReached(MaxPositionsAllowed, MagicNumber, OnJournaling) == False){
      if(EntrySignal(ATR_Current, ATR_Past, LongEntryReversion, ShortEntryReversion)==1){ // Open Long Positions
      ...
      else if(EntrySignal(ATR_Current, ATR_Past, LongEntryReversion, ShortEntryReversion)==2){ // Open Short Positions
...
//| ENTRY SIGNAL                                                     |
//+------------------------------------------------------------------+
int EntrySignal(double p_ATR_Current, double p_ATR_Past, int p_LongEntryReversion, int p_ShortEntryReversion){
// Type: Customisable 
...
// This function checks for entry signals
   ...
   */
   ...
   if (p_ATR_Current < p_ATR_Past){
      ...
      // Long Entry
      if(p_LongEntryReversion == 1){
         entryOutput=1; // Return 1 for Buy Signals
      }

      // Short Entry
      if(p_ShortEntryReversion == 2){
