...
#property link      "blackalgotechnologies.com"
   ...
   /* 
      ...
      BLAIR ENTRY RULES:
      Market Long Order: When low of a hammar candle is < Lower band of Keltner Channels(20) (with k = 2)
      Market Short Order: When high of a hanging man candle is > Upper band of Keltner Channels(20) (with k = 2)
         
      BLAIR EXIT RULES:
      Time-based Exit: Close any order after 10 periods regardless whether in profit or loss
      TakeProfit Order: Place hard TakeProfit order 3 * ATR(20) away from opening price when trade is first executed
...
extern double  TP_ATR_Multiplier = 3;
...
int start()
  {
      ...
      keltnerUpper1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, 2, True, 0, 1);
      keltnerLower1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, 2, True, 2, 1);

      close1 = iClose(NULL, 0, 1);
      close2 = iClose(NULL, 0, 2);

      // Define a variable to identify the candlestick formation 
      /* 
      ...
      */
      ...
      if(CountPosOrders(MagicNumber, OP_BUY)>=1 && ExitSignal(Exit_TimeOut_Period)==3){
         CloseOrderPosition(OP_BUY, OnJournaling, MagicNumber, Slippage, P, RetryInterval); 
      }

      // Close Short Positions
      if(CountPosOrders(MagicNumber, OP_SELL)>=1 && ExitSignal(Exit_TimeOut_Period)==3){
         CloseOrderPosition(OP_SELL, OnJournaling, MagicNumber, Slippage, P, RetryInterval);
      } 

      //----------Entry Rules (Market and Pending) -----------
      
   if(IsLossLimitBreached(IsLossLimitActivated, LossLimitPercent, OnJournaling, EntrySignal(candleType, keltnerLower1, keltnerUpper1)) == False)
   if(IsMaxPositionsReached(MaxPositionsAllowed, MagicNumber, OnJournaling) == False){
      if(EntrySignal(candleType, keltnerLower1, keltnerUpper1)==1){ // Open Long Positions
      ...
      else if(EntrySignal(candleType, keltnerLower1, keltnerUpper1)==2){ // Open Short Positions
...
//| ENTRY SIGNAL                                                     |
//+------------------------------------------------------------------+
int EntrySignal(int p_candleType, double p_keltnerLower1, double p_keltnerUpper1){
// Type: Customisable 
...
// This function checks for exit signals
   ...
   */
   ...
   if(TimeCurrent() > OrderOpenTime() + (p_Exit_TimeOut_Period * Period() * 60) ) //if Order has been opened for more than Exit_TimeOut_Period number of bars //note that OrderOpenTime() requires OrderSelect upstream of calling ExitSignal()
     {
      ExitOutput=3; // Close Long/Short Positions
     }
