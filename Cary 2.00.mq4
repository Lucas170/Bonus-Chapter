...
#property link      "blackalgotechnologies.com"
   ...
   /* 
      ...
      If Current Volatility (ATR(20)) is less than Volatility (ATR(20)) 10 hours ago:
         Market Long Order: closing price crosses upper Keltner Channel(20) from bottom
         Market Short Order: closing price crosses lower Keltner Channel(20) from top

      CARY EXIT RULES:
      Time-based Exit: Close any order after 10 periods regardless whether in profit or loss
      TakeProfit Order: Place hard TakeProfit order 5 * ATR(20) away from opening price when trade is first executed
      StopLoss Order: Place hard StopLoss order 2 * ATR(20) away from opening price when trade is first executed
...
extern int     Entry_ATR_Period        = 20;
extern int     Entry_ATR_Current_Shift = 1;  //Current Volatility
extern int     Entry_ATR_Past_Shift    = 11; //Past Volatility
extern int     Entry_Keltner_Period    = 20;
extern int     Exit_TimeOut_Period     = 10; // Number of bars until trade is exited (used for generic exit rule)
...
extern double  FixedTakeProfit = 0; // Hard Take Profit in Pips. Will be overridden if Is_TP_ATR_On is true 

extern bool    Is_TP_ATR_On      = True; // if this is True, UseFixedTakeProfit must be set to True
extern double  TP_ATR_Multiplier = 5;
extern int     TP_ATR_Period     = 20;
...
int LongEntryBreakout, ShortEntryBreakout;
int OrderNumber;
...
int start()
  {
      ...
      keltnerUpper1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, 1.5, True, 0, 1); //Upper line, shift 1
      keltnerLower1 = iCustom(NULL, 0, "Keltner_Channels", Entry_Keltner_Period, 0, 5, 20, 1.5, True, 2, 1); //Lower line, shift 1
   
      close1 = iClose(NULL, 0, 1);

      ATR_Current = iATR(NULL, 0, Entry_ATR_Period, Entry_ATR_Current_Shift);
      ATR_Past    = iATR(NULL, 0, Entry_ATR_Period, Entry_ATR_Past_Shift);
   
      LongEntryBreakout = Crossed1(close1, keltnerUpper1);
      ShortEntryBreakout = Crossed2(close1, keltnerLower1);

      //----------TP, SL, Breakeven and Trailing Stops Variables-----------
      
      ATR_SL = iATR(NULL, 0, SL_ATR_Period, 1);
      ATR_TP = iATR(NULL, 0, TP_ATR_Period, 1);
      ...
      if(UseFixedTakeProfit == False) {
       ...
       //Take=VolBasedTakeProfit(IsVolatilityTakeProfitOn, FixedTakeProfit, myATR, VolBasedTPMultiplier, P);
         Take=ATRBasedTakeProfit(Is_TP_ATR_On, FixedTakeProfit, ATR_TP, TP_ATR_Multiplier, P);
      }
      ...
      if(CountPosOrders(MagicNumber, OP_BUY)>=1 && ExitSignal(Exit_TimeOut_Period)==1){
         CloseOrderPosition(OP_BUY, OnJournaling, MagicNumber, Slippage, P, RetryInterval); 
      }

      // Close Short Positions
      if(CountPosOrders(MagicNumber, OP_SELL)>=1 && ExitSignal(Exit_TimeOut_Period)==1){
         CloseOrderPosition(OP_SELL, OnJournaling, MagicNumber, Slippage, P, RetryInterval);
      } 

      //----------Entry Rules (Market and Pending) -----------

   if(IsLossLimitBreached(IsLossLimitActivated, LossLimitPercent, OnJournaling, EntrySignal(ATR_Current, ATR_Past, LongEntryBreakout, ShortEntryBreakout)) == False)
   if(IsMaxPositionsReached(MaxPositionsAllowed, MagicNumber, OnJournaling) == False){
      if(EntrySignal(ATR_Current, ATR_Past, LongEntryBreakout, ShortEntryBreakout)==1){ // Open Long Positions
      ...
      else if(EntrySignal(ATR_Current, ATR_Past, LongEntryBreakout, ShortEntryBreakout)==2){ // Open Short Positions
...
//| ENTRY SIGNAL                                                     |
//+------------------------------------------------------------------+
int EntrySignal(double p_ATR_Current, double p_ATR_Past, int p_LongEntryBreakout, int p_ShortEntryBreakout){
// Type: Customisable 
...
// This function checks for entry signals
   ...
   CARY ENTRY RULES:
   If Current Volatility (ATR(20)) is less than Volatility (ATR(20)) 10 hours ago:
   Enter a long trade when closing price crosses upper Keltner Channel(20) from bottom
   Enter a short trade when closing price crosses lower Keltner Channel(20) from top
   */

   int   entryOutput=0;

   if (p_ATR_Current < p_ATR_Past) {

      // Long Entry
      if (p_LongEntryBreakout == 1){
         entryOutput=1; // Return 1 for Buy Signals
      }

      // Short Entry
      if(p_ShortEntryBreakout == 2){
...
//| Exit SIGNAL                                                      |
//+------------------------------------------------------------------+
int ExitSignal(int p_Exit_TimeOut_Period){
// Type: Customisable 
// Modify this function to suit your trading robot
...
// This function checks for exit signals
   ...
   */
   ...
   //Generic Exit
   if(TimeCurrent() > OrderOpenTime() + (p_Exit_TimeOut_Period * Period() * 60) ) //if Order has been opened for more than Exit_TimeOut_Period number of bars //note that OrderOpenTime() requires OrderSelect upstream of calling ExitSignal()
     {
      ExitOutput=1; // Close Long/Short Positions
     }
...
//+------------------------------------------------------------------+
double ATRBasedTakeProfit(bool p_Is_TP_ATR_On, double p_FixedTakeProfit, double p_ATR_TP, double p_TP_ATR_Multiplier, int K){ // K represents our P multiplier to adjust for broker digits
   if(!p_Is_TP_ATR_On){
      return(p_FixedTakeProfit); // If Volatility Stop Loss not activated. Stop Loss = Fixed Pips Stop Loss
   }
   else{
      return(p_TP_ATR_Multiplier * p_ATR_TP / (K*Point)); // TakeProfit in Pips
   }
}
